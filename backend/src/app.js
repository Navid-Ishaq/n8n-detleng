...
export function validateN8nCloudWebhookUrl(value) {
  if (typeof value !== 'string' || value.length > 2048) return { valid: false, reason: 'Enter a valid n8n Production Webhook URL.' }
  ...
  if (url.pathname.includes('/webhook-test/')) return { valid: false, reason: 'Use the Production URL, not the n8n Test URL.' }
  if (!url.pathname.includes('/webhook/')) return { valid: false, reason: 'The URL must be an n8n Production Webhook URL.' }
...
async function runWebhookTest(fetchImpl, webhookUrl, testCase, timeoutMs) {
  try {
    ...
    if (!response.ok) {
      return { id: testCase.id, label: testCase.label, passed: false, expected: testCase.expected, message: `The webhook returned HTTP ${response.status}.` }
    }

    const contentType = response.headers.get('content-type') ?? ''
    if (!contentType.toLowerCase().includes('application/json')) {
      return { id: testCase.id, label: testCase.label, passed: false, expected: testCase.expected, message: 'The webhook response was not JSON.' }
    }

    let body
    try { body = await response.json() } catch {
      return { id: testCase.id, label: testCase.label, passed: false, expected: testCase.expected, message: 'The webhook returned invalid JSON.' }
    }
    ...
    return { id: testCase.id, label: testCase.label, passed: false, expected: testCase.expected, message: timedOut ? 'The n8n webhook did not respond before the timeout.' : 'The n8n webhook could not be reached safely.' }
  }
}
...
export function createApp(options = {}) {
  ...
  app.post('/api/lessons/n8n-core/live-test', liveTestLimiter, async (request, response) => {
    ...
    if (!match) return response.status(401).json({ error: 'Please log in to run a live test.' })

    const { data, error } = await supabase.auth.getUser(match[1])
    if (error || !data?.user) return response.status(401).json({ error: 'Your session is no longer valid. Please log in again.' })
