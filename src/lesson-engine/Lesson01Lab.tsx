...
export function Lesson01Lab() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (current.error) { console.error('Lesson 01 V2 hydration failed', current.error); const fallback = cached?.progress ?? structuredClone(lesson01InitialProgress); progressRef.current = fallback; setProgress(fallback); setSyncPending(true); setSaveState('Your progress is saved locally and will sync when the connection is restored.'); setLoading(false); return }
      ...
      progressRef.current = next; setProgress(next); setLoading(false); if (!current.data || cached?.syncPending) void persist(next)
    })
  }, [persist, user])
  ...
  async function connectProduction() {
    ...
    setFeedback({ passed: true, message: 'Production workflow connected.' }); updateProgress({ productionConnected: true, lastNeedsAttention: false, stageStates: markPassed(progress, 'publish-connect') })
  }
  ...
  return <div className="portal-page"><header className="portal-header"><a className="back-link" href="/dashboard"><ArrowLeft size={18}/> Dashboard</a><div className="portal-header-actions"><a className="brand brand--compact" href="/"><span className="brand-mark"><Network size={19}/></span><span>n8n <strong>Detleng</strong></span></a><button className="button button--ghost" onClick={() => void logout()}><LogOut size={17}/> Log out</button></div></header>
    <main className="portal-main lab-shell"><div className="lab-hero"><div><p className="eyebrow">Lesson 01 · Live foundation lab · V2</p><h1>n8n Core</h1><p className="portal-lede">Build, publish and debug an Operations Intake Workflow while Detleng tests your real n8n.</p></div><div className={`lab-status lab-status--${status.toLowerCase().replaceAll(' ', '-')}`}><span>Automatic status</span><strong>{status}</strong><small>{saveState || 'Progress syncs to your learner account.'}</small>{syncPending && <button className="sync-retry" onClick={() => void persist(progressRef.current, progressRef.current.completed)}>Retry sync</button>}</div></div>
      ...
      <section className="lab-panel">
        ...
        {stage === 'complete' && <Complete items={completionItems} completed={progress.completed} enabled={canComplete} onComplete={() => void completeLesson()}/>} 
...
function Complete({ items,completed,enabled,onComplete }: { items:readonly (readonly [string,boolean])[]; completed:boolean; enabled:boolean; onComplete:()=>void }) { return <div><StageHeader number="08" title="Your engineering summary" text="Completion unlocks only after the real workflow has passed the full build, break and repair cycle."/><div className="completion-grid">{items.map(([label,passed])=><div className={passed?'completion-item is-pass':'completion-item'} key={label}><span>{passed?'✓':'○'}</span>{label}</div>)}</div><button className="button button--primary complete-button" disabled={!enabled||completed} onClick={onComplete}>{completed?'✓ Lesson 01 Completed':enabled?'Complete Lesson 01':'Complete the live lab first'}</button></div> }
