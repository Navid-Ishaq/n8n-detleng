...
export function normalizeLesson01Progress(value: unknown): Lesson01Progress {
  ...
  return { ...structuredClone(lesson01InitialProgress), ...saved, viewedStage: saved.viewedStage || saved.currentStage || 'understand', stageStates: { ...lesson01InitialProgress.stageStates, ...saved.stageStates } }
}
...
export function advanceLesson01Progress(current: Lesson01Progress, patch: Partial<Lesson01Progress>) { return { ...current, ...patch, revision: current.revision + 1, clientUpdatedAt: new Date().toISOString() } }
